...
[foremanctl](https://github.com/theforeman/foremanctl) is the deployment tool for Foreman and IoP, using Podman quadlets and Ansible. IoP is enabled by adding `iop` to `enabled_features` in the flavor configuration. It requires internal database mode (`database_mode: internal`) and depends on the `rh-cloud` feature (which installs the `foreman_rh_cloud` plugin).
...
All IoP images default to `quay.io/iop/<service>:foreman-3.18`. Each role exposes `iop_<role>_container_image` and `iop_<role>_container_tag` variables to override the image.

#### Container Network

IoP services run on the `iop-core-network` bridge network (`10.130.0.0/24`). Containers access the host PostgreSQL instance via `host.containers.internal:5432`.

#### Engine Rule Packages

The Insights engine loads Python rule packages listed in `iop_engine_packages`. A separate `iop_engine_extra_packages` list (default: `[]`) is available for downstream deployments to add packages that are not present in the community images:
...
### Legacy: puppet-iop Configuration

:::note
[puppet-iop](https://github.com/theforeman/puppet-iop/) is the legacy deployment method using Foreman Installer and Puppet. It is being replaced by [foremanctl](https://github.com/theforeman/foremanctl).
:::

The unit files, including timers and network, can be generated by Foreman Installer via [puppet-iop](https://github.com/theforeman/puppet-iop/). Each service has a Puppet module defined in [manifests](https://github.com/theforeman/puppet-iop/tree/master/manifests). The Puppet modules define image names through parameters that can be overridden with Hiera (a Puppet variable configuration method).
